...
Recibe los leads de la landing (`fiflip-landing`) por HTTP y, en el mismo
request, manda un email de notificación y un evento a la Meta Conversions
API por cada lead recibido.

```
Formulario (React) → POST /api/leads/* → email (Resend) + Meta CAPI (síncrono)
```
